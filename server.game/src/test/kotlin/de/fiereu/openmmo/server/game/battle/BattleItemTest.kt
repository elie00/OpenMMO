...
class BattleItemTest :
    FunSpec({
      test("Master Ball guarantees 100% catch rate") {
        ...
        CatchCalculator.attemptCatch(Items.MASTER_BALL, target, turn = 1, BattleRng(seed = 123)).shouldBeTrue()
      }
      ...
      test("Weakened and asleep Pokémon is easier to catch") {
        ...
        CatchCalculator.attemptCatch(Items.POKE_BALL, target, turn = 1, BattleRng(seed = 42)).shouldBeTrue()
      }
    })
