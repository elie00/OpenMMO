...
constructor(
  ...
  private fun applyHealingItem(
      ...
      when (itemId) {
        Items.POTION -> {
          if (target.fainted || target.currentHp >= target.stats.hp) {
            false
          } else {
            val healed = 20
        ...
        Items.FULL_RESTORE -> {
          if (target.fainted ||
            ...
            emitter.sendNotice(
                battle, "${target.species.name} fully restored HP and cured all status!")
            true
          }
        }
        Items.ANTIDOTE -> {
        ...
        Items.BURN_HEAL -> {
          if (target.primaryStatus != PrimaryStatus.BURN) {
            false
          } else {
            target.cureStatus()
            emitter.broadcast(
        ...
        Items.ICE_HEAL -> {
          if (target.primaryStatus != PrimaryStatus.FREEZE) {
            false
          } else {
            target.cureStatus()
            emitter.broadcast(
        ...
        Items.AWAKENING -> {
          if (target.primaryStatus != PrimaryStatus.SLEEP) {
            false
          } else {
            target.cureStatus()
            emitter.broadcast(
        ...
        Items.PARALYZE_HEAL -> {
          if (target.primaryStatus != PrimaryStatus.PARALYSIS) {
            false
          } else {
            target.cureStatus()
            emitter.broadcast(
        ...
        Items.FULL_HEAL -> {
          if (target.primaryStatus == PrimaryStatus.NONE && !target.isConfused) {
            false
          } else {
            target.cureStatus()
            target.cureVolatiles()
  ...
  private fun checkIntimidateOnEntry(
    ...
    if (incoming.ability == Ability.INTIMIDATE &&
        !opponent.fainted &&
        opponent.ability != Ability.CLEAR_BODY &&
        opponent.ability != Ability.WHITE_SMOKE) {
