...
internal object PokemonLeague_AgathasRoom_EventScript_Agatha : Script {
  override suspend fun run(ctx: ScriptContext) {
    if (ctx.isFlagSet(KantoFlags.FLAG_DEFEATED_AGATHA)) {
      return ctx.say(PokemonLeague_AgathasRoom.PostBattle)
    }
    ctx.say(PokemonLeague_AgathasRoom.Intro)
    // trainerbattle_no_intro: the intro box above is theirs.
    if (ctx.trainerBattle(KantoTrainerIds.TRAINER_ELITE_FOUR_AGATHA) != BattleResult.VICTORY) return
    ctx.say(PokemonLeague_AgathasRoom.Defeat)
